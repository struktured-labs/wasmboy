...
function _prepareAndStoreAutoSave() {
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      saveState: saveState,
      // Self-describing, since recovery runs before the storage key is
      // reconfigured on the next load.
      cartridgeRamStorageKey: this.cartridgeRamStorageKey,
      cartridgeRamStorageSize: this.cartridgeRamStorageSize,
      cartridgeRamSavedAt: Date.now()
    })
  );
...
function _findAndCommitAutoSave() {
  const findAndCommitAutoSaveTask = async () => {
    ...
    if (unloadStorage) {
      ...
      // Battery RAM was captured under a stable key: dual-write it (stable +
      // header) so a rollback still finds it. The config came from localStorage,
      // so validate it first; on anything malformed, fall back to the header
      // save rather than lose the RAM. Save states stay revision-specific.
      const key = unloadStorageObject.cartridgeRamStorageKey;
      const size = unloadStorageObject.cartridgeRamStorageSize;
      const rawSavedAt = unloadStorageObject.cartridgeRamSavedAt;
      const savedAt = Number.isFinite(rawSavedAt) && rawSavedAt >= 0 ? rawSavedAt : Date.now();
      if (key !== undefined && key !== null && this._isStableRamConfigValid(key, size)) {
        await this._persistCartridgeRam(cartridgeRam, savedAt, key, size, header);
      } else {
