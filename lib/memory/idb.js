...
if (typeof window !== 'undefined') {
  ...
  keyval = {
    ...
    // Read, apply, write in one transaction so a concurrent writer cannot slip
    // between the read and the write. The updater returns the value to store, or
    // undefined to leave the record untouched.
    update(key, updater) {
      return dbPromise.then(db => {
        const tx = db.transaction('keyval', 'readwrite');
        const store = tx.objectStore('keyval');
        return store.get(key).then(current => {
