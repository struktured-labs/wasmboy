...
class WasmBoyMemoryService {
  ...
  loadState(saveState) {
    const loadStateTask = async () => {
      ...
      // caller's state is emptied by the load, and loading it again silently
      // restores nothing.
      const toTransferableBuffer = memoryValue => new Uint8Array(memoryValue).slice().buffer;

      const workerMemoryObject = {};
      workerMemoryObject[MEMORY_TYPE.CARTRIDGE_RAM] = toTransferableBuffer(saveState.wasmboyMemory.cartridgeRam);
      workerMemoryObject[MEMORY_TYPE.GAMEBOY_MEMORY] = toTransferableBuffer(saveState.wasmboyMemory.gameBoyMemory);
      workerMemoryObject[MEMORY_TYPE.PALETTE_MEMORY] = toTransferableBuffer(saveState.wasmboyMemory.wasmBoyPaletteMemory);
      workerMemoryObject[MEMORY_TYPE.INTERNAL_STATE] = toTransferableBuffer(saveState.wasmboyMemory.wasmBoyInternalState);
