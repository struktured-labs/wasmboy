...
class WasmBoyMemoryService {
  ...
  _fetchSaveStateMemoryIfMissing() {
    const fetchTask = async () => {
      // Length, not just presence: a previous loadState transfers these
      // buffers away, which leaves a detached view behind. That is still an
      // object, but reading it yields nothing.
      const isPopulated = memoryValue => memoryValue && memoryValue.length > 0;
      if (isPopulated(this.cartridgeRam) && isPopulated(this.gameboyMemory) && isPopulated(this.paletteMemory) && isPopulated(this.internalState)) {
        return;
      }
