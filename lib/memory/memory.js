...
import {
  ...
  shouldReplaceRam,
  titleFromRecord,
  chooseMigrationSource
} from './ram-storage.js';

// The wasm cartridge RAM backing, from core/constants.ts CARTRIDGE_RAM_SIZE.
const CARTRIDGE_RAM_BACKING_SIZE = 0x020000;

const BOOT_ROM_KEY = 'boot-rom-';

function getSaveStateMemoryArray(memoryValue, memoryKey) {
  if (memoryValue instanceof Uint8Array) {
    return memoryValue;
  }

  if (memoryValue instanceof ArrayBuffer) {
    return new Uint8Array(memoryValue);
  }

  if (ArrayBuffer.isView(memoryValue)) {
    return new Uint8Array(memoryValue.buffer, memoryValue.byteOffset, memoryValue.byteLength);
  }

  if (Array.isArray(memoryValue)) {
    return Uint8Array.from(memoryValue);
  }

  if (memoryValue && typeof memoryValue === 'object') {
    const numericKeys = Object.keys(memoryValue)
      .filter(key => String(parseInt(key, 10)) === key)
      .sort((a, b) => parseInt(a, 10) - parseInt(b, 10));

    if (numericKeys.length > 0) {
      const typedArray = new Uint8Array(numericKeys.length);
      numericKeys.forEach((key, index) => {
        typedArray[index] = memoryValue[key];
      });
      return typedArray;
    }
  }

  throw new Error(`Invalid save state memory for ${memoryKey}`);
}
...
class WasmBoyMemoryService {
  constructor() {
    ...
    // Optional stable IndexedDB key for battery RAM, so a save survives ROM
    // revisions whose changed header would otherwise strand it. Off by default.
    this.cartridgeRamStorageKey = undefined;
    this.cartridgeRamStorageSize = undefined;
  ...
  clearMemory(randomizeStartupRam) {
    // Clear Wasm memory
    // https://docs.google.com/spreadsheets/d/17xrEzJk5-sCB9J2mMJcVnzhbE-XH_NvczVSQH9OHvRk/edit?usp=sharing
    // Carried on the message because clearing happens before config, so the
    // worker has no options to consult yet.
    return this.worker
      .postMessage({
        type: WORKER_MESSAGE_TYPE.CLEAR_MEMORY,
        randomizeStartupRam: Boolean(randomizeStartupRam)
      })
      .then(event => {
  ...
  saveLoadedCartridge(additionalInfo) {
    const saveLoadedCartridgeRomTask = async () => {
      ...
      // Fold current RAM + its capture time into the cached object BEFORE the
      // set() below, so the write cannot resurrect the stale RAM read at the top.
      if (this.cartridgeRam) {
        const savedAt = Date.now();
        if (this.cartridgeRamStorageKey) {
          const cropped = cropCartridgeRam(this.cartridgeRam, this.cartridgeRamStorageSize);
          cartridgeObject.cartridgeRam = cropped;
          cartridgeObject.cartridgeRamSavedAt = savedAt;
          // Mirror the exact same bytes/time under the stable key (guarded).
          await this._writeStableRam(this.cartridgeRamStorageKey, cropped, savedAt);
        } else {
          cartridgeObject.cartridgeRam = this.cartridgeRam;
          cartridgeObject.cartridgeRamSavedAt = savedAt;
        }
      }
  ...
  // Configure stable-key battery RAM storage. Ignored unless a key and a valid
  // size within the backing are given, so the default stays header-keyed.
  configureCartridgeRamStorage(key, size) {
    const keyGiven = key !== undefined && key !== null;
    const sizeGiven = size !== undefined && size !== null;
    if (!keyGiven && !sizeGiven) {
      this.cartridgeRamStorageKey = undefined;
      this.cartridgeRamStorageSize = undefined;
      return;
    }
    // Supplying one but not both, or an invalid value, must fail rather than
    // silently fall back to header-keyed storage and re-strand saves.
    if (typeof key !== 'string' || key.length === 0) {
      throw new Error('cartridgeRamStorageKey must be a non-empty string');
    }
    if (!isValidStorageSize(size, CARTRIDGE_RAM_BACKING_SIZE)) {
      throw new Error(`cartridgeRamStorageSize must be an integer between 1 and ${CARTRIDGE_RAM_BACKING_SIZE}`);
    }
    this.cartridgeRamStorageKey = key;
    this.cartridgeRamStorageSize = size;
  }

  // Stable record is RAM-only: replace it wholesale, never merge foreign fields.
  _writeStableRam(key, cropped, savedAt) {
    return idbKeyval.update(key, existing => {
      if (!shouldReplaceRam(existing, savedAt)) {
        return undefined;
      }
      return { cartridgeRam: cropped, cartridgeRamSavedAt: savedAt };
    });
  }

  // Header record carries metadata and save states: merge RAM in, preserve rest.
  _mergeHeaderRam(header, cropped, savedAt) {
    return idbKeyval.update(header, existing => {
      if (!shouldReplaceRam(existing, savedAt)) {
        return undefined;
      }
      const record = existing || {};
      record.cartridgeRam = cropped;
      record.cartridgeRamSavedAt = savedAt;
      return record;
    });
  }

  // Whether a (key, size) pair is a usable stable-storage config: a non-empty
  // string key and an integer size within the cartridge RAM backing. Used to
  // gate autosave recovery, whose config comes from untrusted localStorage.
  _isStableRamConfigValid(key, size) {
    return typeof key === 'string' && key.length > 0 && isValidStorageSize(size, CARTRIDGE_RAM_BACKING_SIZE);
  }

  // Dual-write RAM to the stable record and the header record under one capture
  // time, so a rollback to an older bundle that reads the header still finds it.
  async _persistCartridgeRam(cartridgeRam, savedAt, key, size, header) {
    const storageKey = typeof key === 'string' && key.length > 0 ? key : this.cartridgeRamStorageKey;
    const storageSize = isValidStorageSize(size, CARTRIDGE_RAM_BACKING_SIZE) ? size : this.cartridgeRamStorageSize;
    if (typeof storageKey !== 'string' || storageKey.length === 0 || !isValidStorageSize(storageSize, CARTRIDGE_RAM_BACKING_SIZE)) {
      throw new Error('Invalid cartridge RAM storage key or size');
    }
    const cropped = cropCartridgeRam(cartridgeRam, storageSize);
    await this._writeStableRam(storageKey, cropped, savedAt);
    if (header) {
      await this._mergeHeaderRam(header, cropped, savedAt);
    }
  }

  // The newest same-title record that carries RAM, for seeding the stable key
  // the first time a game runs under it (including users who skipped revisions
  // and legacy records that have RAM but no cartridgeRom metadata).
  async _findMigrationRam() {
    const wantTitle = titleFromRecord(undefined, this.cartridgeHeader);
    if (!wantTitle) {
      return undefined;
    }
    ...
    for (const key of keys) {
      if (key === this.cartridgeRamStorageKey || typeof key === 'string') {
        continue;
      }
      const record = await idbKeyval.get(key);
      if (record && record.cartridgeRam !== undefined && titleFromRecord(record, key) === wantTitle) {
        candidates.push({ record });
      }
    }

    const source = chooseMigrationSource(candidates);
    return source ? { cartridgeRam: source.record.cartridgeRam, savedAt: ramTimestamp(source.record) } : undefined;
  }
  ...
  saveCartridgeRam(passedHeader, passedCartridgeRam) {
    const saveCartridgeRamTask = async () => {
      ...
      // Stable key set: dual-write RAM to stable + header (see _persistCartridgeRam).
      if (this.cartridgeRamStorageKey) {
        await this._persistCartridgeRam(cartridgeRam, Date.now(), undefined, undefined, header);
        return;
      }

      // Default (header-keyed) path. Stamp the capture time so a later migration
      // ranks by when the RAM was saved, not by unrelated metadata writes.
      await idbKeyval.update(header, existing => {
        const record = existing || {};
        record.cartridgeRam = cartridgeRam;
        record.cartridgeRamSavedAt = Date.now();
        return record;
      });
    };
  ...
  loadCartridgeRam() {
    const loadCartridgeRamTask = async () => {
      ...
      let cartridgeRam;
      if (this.cartridgeRamStorageKey) {
        // Always weigh the stable record against the newest same-title legacy
        // record: an old bundle can commit a newer header-keyed autosave after
        // a stable record already exists. Newer wins; an exact tie keeps stable.
        const stable = await idbKeyval.get(this.cartridgeRamStorageKey);
        const stableSavedAt = stable && stable.cartridgeRam !== undefined ? ramTimestamp(stable) : -1;
        const migrated = await this._findMigrationRam();

        if (migrated && migrated.savedAt > stableSavedAt) {
          // Adopt the legacy save into the stable record (and mirror it into the
          // current header for rollback), preserving its original capture time
          // so a later, genuinely-newer save still wins.
          await this._persistCartridgeRam(migrated.cartridgeRam, migrated.savedAt, undefined, undefined, header);
          cartridgeRam = cropCartridgeRam(migrated.cartridgeRam, this.cartridgeRamStorageSize);
        } else if (stable && stable.cartridgeRam !== undefined) {
          cartridgeRam = cropCartridgeRam(stable.cartridgeRam, this.cartridgeRamStorageSize);
        }
      } else {
        const cartridgeObject = await idbKeyval.get(header);
        if (cartridgeObject && cartridgeObject.cartridgeRam) {
          cartridgeRam = cartridgeObject.cartridgeRam;
        }
      }

      if (cartridgeRam === undefined) {
        return;
      }
      ...
      const workerMemoryObject = {};
      workerMemoryObject[MEMORY_TYPE.CARTRIDGE_RAM] = cartridgeRam.buffer;
      await this.worker
        .postMessage({
          type: WORKER_MESSAGE_TYPE.SET_MEMORY,
          ...workerMemoryObject
        ...
        .then(event => {
          ...
          this.cartridgeRam = cartridgeRam;
        });
    };
  ...
  // The worker only pushes memory back to us as part of the play loop. Code
  // that drives frames directly, which headless callers do, never sees that
  // push, so ask for the memory instead of saving a state full of undefined.
  _fetchSaveStateMemoryIfMissing() {
    const fetchTask = async () => {
      const hasMemory = this.cartridgeRam && this.gameboyMemory && this.paletteMemory && this.internalState;
      if (hasMemory) {
        return;
      }

      const event = await this.worker.postMessage({
        type: WORKER_MESSAGE_TYPE.GET_MEMORY,
  ...
  saveState(passedHeader, passedSaveState) {
    const saveStateTask = async () => {
      ...
      if (passedHeader && passedSaveState) {
        ...
        await this._fetchSaveStateMemoryIfMissing();
  ...
  loadState(saveState) {
    const loadStateTask = async () => {
      ...
      const saveStateMemory = saveState.wasmboyMemory;

      // Copy before handing these to the worker. They go across as
      // transferables, which detaches the underlying buffer on this side, and
      // the buffer belongs to the caller's save state. Without the copy, the
      // caller's state is emptied by the load: restoring the same checkpoint a
      // second time then silently restores nothing instead of failing.
      const toTransferableBuffer = (memoryValue, memoryKey) => getSaveStateMemoryArray(memoryValue, memoryKey).slice().buffer;

      const workerMemoryObject = {};
      workerMemoryObject[MEMORY_TYPE.CARTRIDGE_RAM] = toTransferableBuffer(saveStateMemory.cartridgeRam, 'cartridgeRam');
      workerMemoryObject[MEMORY_TYPE.GAMEBOY_MEMORY] = toTransferableBuffer(saveStateMemory.gameBoyMemory, 'gameBoyMemory');
      workerMemoryObject[MEMORY_TYPE.PALETTE_MEMORY] = toTransferableBuffer(saveStateMemory.wasmBoyPaletteMemory, 'wasmBoyPaletteMemory');
      workerMemoryObject[MEMORY_TYPE.INTERNAL_STATE] = toTransferableBuffer(saveStateMemory.wasmBoyInternalState, 'wasmBoyInternalState');
      ...
      await this.worker
        ...
        .then(event => {
          const eventData = getEventData(event);
          // Wrap these the way every other memory path does. Left as raw
          // ArrayBuffers, the next save state comes back with a different
          // shape than one taken before a load, and does not survive JSON.
          this.cartridgeRam = new Uint8Array(eventData.message[MEMORY_TYPE.CARTRIDGE_RAM]);
          this.gameboyMemory = new Uint8Array(eventData.message[MEMORY_TYPE.GAMEBOY_MEMORY]);
          this.paletteMemory = new Uint8Array(eventData.message[MEMORY_TYPE.PALETTE_MEMORY]);
          this.internalState = new Uint8Array(eventData.message[MEMORY_TYPE.INTERNAL_STATE]);
