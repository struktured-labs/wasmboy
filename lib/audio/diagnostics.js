...
export function getProducerPacing() {
  return {
    ...
    // With acknowledgements the queue is measured, not modelled. ackActive
    // false means the producer is still estimating, which on the direct path
    // means acknowledgements are not getting through.
    acceptedQueuedSeconds: producerExtra.acceptedQueuedSeconds,
    ackActive: Boolean(producerExtra.ackActive),
    ackTimeouts: producerExtra.ackTimeouts || 0,
    blocksInFlight: producerExtra.blocksInFlight,
    unackedSeconds: producerExtra.unackedSeconds,
    lastAckSequence: producerExtra.lastAckSequence,
    // The largest single accepted increment seen, which bounds how far above
    // target the queue can sit.
    lastBlockFrames: producerExtra.lastBlockFrames,
    maxBlockFrames: producerExtra.maxBlockFrames,
    // 'ack-ledger' when the acknowledgement ledger is driving pacing,
    // 'status' when it has fallen back to the periodic status projection.
    pacingSource: producerExtra.pacingSource
  };
}
