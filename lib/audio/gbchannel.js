...
export default class GbChannelWebAudio {
  ...
  playAudio(numberOfSamples, leftChannelBuffer, rightChannelBuffer, playbackRate, updateAudioCallback) {
    ...
    if (this.audioWorkletReady) {
      this._playAudioWorklet(
        numberOfSamples,
        leftChannelBufferAsFloat,
        rightChannelBufferAsFloat,
        playbackRate
      );
      return;
    }
  ...
  prepareAudioOutput(updateAudioCallback) {
    this.createAudioContextIfNone();
  ...
  _playAudioWorklet(numberOfSamples, left, right, playbackRate) {
    ...
    this.audioWorkletLatencySeconds = Math.min(
      capacityLatency,
      this.audioWorkletLatencySeconds + latency
    );
    this.audioWorkletNode.port.postMessage(
      {
        'type': 'write',
