...
import { getEffectiveTargetLatencySeconds } from './pacing';
import { buildAudioDiagnostics, getProducerPacing, resetProducerPacing } from './diagnostics';
...
class WasmBoyAudioService {
  ...
  initialize(options) {
    const initializeTask = async () => {
      if (typeof options === 'function') {
        options = { updateAudioCallback: options };
      }
      options = options || {};
      this.updateAudioCallback = options.updateAudioCallback;

      this.averageTimeStretchFps = [];

      this.speed = 1.0;

      // Debug channels stay lazy: only build the extra contexts when the
      // debugger actually asked for them.
      // Remembered so the diagnostics describe the run rather than being
      // reconstructed from fields nobody assigned.
      this.audioOptions = options;
      this.workletActive = false;
      this.directOutputActive = false;
      resetProducerPacing();

      const isDirectOutput = Boolean(options.audioWorkletDirectOutput) && !options.enableAudioDebugging;
      this.effectiveTargetLatencySeconds = getEffectiveTargetLatencySeconds(options.audioTargetLatencyInSeconds, isDirectOutput);

      this._createAudioContextIfNone(options.enableAudioDebugging);
      const workletReady = await this.gbChannels.master.prepareAudioOutput(this.updateAudioCallback, this.effectiveTargetLatencySeconds);
      this.workletActive = Boolean(workletReady);
      if (workletReady && isDirectOutput) {
        const audioInputPort = this.gbChannels.master.createAudioInputPort();
        if (audioInputPort) {
          this.directOutputActive = true;
          this.worker.postMessageIgnoreResponse(
            {
              type: WORKER_MESSAGE_TYPE.CONNECT_AUDIO_OUTPUT,
              ports: [audioInputPort]
            },
            [audioInputPort]
          );
        }
  ...
  // What the audio path is actually doing right now. A harness measuring
  // latency needs to know which path it got, not which one it asked for: the
  // worklet can fail to load, and direct output is refused while the audio
  // debugger is on.
  getDiagnostics() {
    const master = this.gbChannels && this.gbChannels.master;
    return buildAudioDiagnostics({
      options: this.audioOptions,
      workletActive: this.workletActive,
      directOutputActive: this.directOutputActive,
      effectiveTargetLatencySeconds: this.effectiveTargetLatencySeconds,
      workletStats: master ? master.audioWorkletStats : undefined,
      producer: getProducerPacing()
    });
  }
  ...
  setSpeed(speed) {
    this.speed = speed;
    this.gbChannels.master.setSpeed(speed);
  ...
  _createAudioContextIfNone(includeDebugChannels) {
    this.gbChannels.master.createAudioContextIfNone();
    if (includeDebugChannels) {
      for (let channelNumber = 1; channelNumber <= 4; channelNumber++) {
        this.gbChannels[`channel${channelNumber}`].createAudioContextIfNone();
      }
    }
  }
