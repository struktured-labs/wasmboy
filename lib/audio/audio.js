...
class WasmBoyAudioService {
  ...
  initialize(options) {
    const initializeTask = async () => {
      ...
      const workletReady = await this.gbChannels.master.prepareAudioOutput(this.updateAudioCallback, options.audioTargetLatencyInSeconds);
      if (workletReady && options.audioWorkletDirectOutput && !options.enableAudioDebugging) {
  ...
  getDiagnostics() {
    ...
    return {
      workletActive: Boolean(this.workletActive),
      directOutputRequested: Boolean(this.requestedDirectOutput),
      directOutputActive: Boolean(this.directOutputActive),
      effectiveTargetLatencySeconds: this.effectiveTargetLatencySeconds,
      worklet: master ? master.audioWorkletStats : undefined
    };
  }
