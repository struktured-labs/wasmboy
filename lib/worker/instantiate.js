...
const getConfiguredWorkerUrl = (workerUrls, workerKey, defaultWorkerUrl) => {
  if (workerUrls && workerUrls[workerKey]) {
    return workerUrls[workerKey];
  }

  return defaultWorkerUrl;
};

export const instantiateWorkers = async workerUrls => {
  // Create our workers
  let libWorkerUrl;
  libWorkerUrl = wasmboyLibWasmWorkerUrl;
  libWorkerUrl = wasmboyLibTsWorkerUrl;

  const libWorker = new SmartWorker(getConfiguredWorkerUrl(workerUrls, 'lib', libWorkerUrl), WORKER_ID.LIB);
  const graphicsWorker = new SmartWorker(getConfiguredWorkerUrl(workerUrls, 'graphics', wasmboyGraphicsWorkerUrl), WORKER_ID.GRAPHICS);
  const audioWorker = new SmartWorker(getConfiguredWorkerUrl(workerUrls, 'audio', wasmboyAudioWorkerUrl), WORKER_ID.AUDIO);
  const controllerWorker = new SmartWorker(
    getConfiguredWorkerUrl(workerUrls, 'controller', wasmboyControllerWorkerUrl),
    WORKER_ID.CONTROLLER
  );
  const memoryWorker = new SmartWorker(getConfiguredWorkerUrl(workerUrls, 'memory', wasmboyMemoryWorkerUrl), WORKER_ID.MEMORY);
