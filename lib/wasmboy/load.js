...
async function initialize() {
  ...
  await WasmBoyMemory.clearMemory(this.options.randomizeStartupRam);
...
export function loadROMToWasmBoy(ROM, fetchHeaders) {
  const loadROMAndConfigTask = async () => {
    ...
    const fetchROMObject = await fetchROMAsByteArray(ROM, {
      ...(fetchHeaders || {}),
    ...
    await this.worker.postMessage({
      ...
      options: {
        ...
        randomizeStartupRam: this.options.randomizeStartupRam,
        audioTargetLatencyInSeconds: this.options.audioTargetLatencyInSeconds
      }
    });
  };
    ...
    if (this.options.headless) {
      ...
      await Promise.all([
        ...
        WasmBoyAudio.initialize({
          updateAudioCallback: this.options.updateAudioCallback,
          enableAudioDebugging: this.options.enableAudioDebugging,
          audioWorkletDirectOutput: this.options.audioWorkletDirectOutput,
          audioTargetLatencyInSeconds: this.options.audioTargetLatencyInSeconds
        }),
      ...
      // Configure stable-key battery RAM before restoring it, so a save made
      // under a previous ROM revision is found.
      WasmBoyMemory.configureCartridgeRamStorage(this.options.cartridgeRamStorageKey, this.options.cartridgeRamStorageSize);
