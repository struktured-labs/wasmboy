...
const loadROMBytes = async (bytes, fileName) => {
...
const loadROMFilePathAsByteArray = async filePath => {
  /*ROLLUP_REPLACE_NODE
  const fs = require('fs');

  const bytes = fs.readFileSync(filePath);
  return loadROMBytes(bytes, filePath);
  ROLLUP_REPLACE_NODE*/

  throw new Error('Loading a ROM from a file path requires Node');
};
...
export const fetchROMAsByteArray = (ROM, loadOptions) => {
  const fetchROMAsByteArrayTask = async () => {
    ...
    if (ArrayBuffer.isView(ROM) && ROM.constructor === Uint8Array) {
      ...
      if ((loadOptions && loadOptions.headless) || typeof fetch === 'undefined') {
        return loadROMFilePathAsByteArray(ROM);
      }
      ...
      return loadROMBytes(bytes, fileName);
    }
  };
