...
// Backpressure only. This is the point at which the emulator stops handing the
// audio path more work, not the latency we aim to run at: when the worklet is
// in use it holds the queue at its own, lower target and never reaches this.
// Kept above that target so the two do not fight over the same queue.
const DEFAULT_MAX_AUDIO_LATENCY = 0.045;

const getMaxAudioLatency = libWorker => {
  const target = libWorker.options && libWorker.options.audioTargetLatencyInSeconds;
  if (!target || target <= 0) {
    return DEFAULT_MAX_AUDIO_LATENCY;
  }
  return Math.max(DEFAULT_MAX_AUDIO_LATENCY, target + 0.015);
};
...
function executeAndCheckAudio(libWorker, resolve) {
  ...
  if (response === 1) {
    ...
    const maxAudioLatency = getMaxAudioLatency(libWorker);
    const isTooMuchLatency = libWorker.currentAudioLatencyInSeconds > maxAudioLatency;
    if (isTooMuchLatency) {
      // Let the browser audio clock consume the full excess before queuing
      // another block. The old one-tenth delay let video continually outrun
      // audio, making the A/V offset grow throughout play.
      const latencyDifferenceInSeconds = libWorker.currentAudioLatencyInSeconds - maxAudioLatency;
