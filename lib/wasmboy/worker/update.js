...
const getUnackedSeconds = libWorker =>
  (libWorker.audioInFlightBlocks || []).reduce((total, block) => total + block.frames, 0) / AUDIO_SAMPLE_RATE;
...
const shouldWaitForAudioCredit = libWorker => {
  ...
  const admission = evaluateAdmission({
    ...
    oldestWaitedMs: oldest ? getPerformanceTimestamp() - oldest.sentAt : 0
  });
    ...
    if (response >= 0) {
      ...
      postMessage(
        getSmartWorkerMessage({
          ...
          audioBlocksInFlight: (libWorker.audioInFlightBlocks || []).length,
...
function executeAndCheckAudio(libWorker, resolve) {
  ...
  if (response === 1) {
    ...
    // Acknowledgements are the window, not the pacing signal. An
    // acknowledgement is timestamped when it arrives, but its measurement was
    // taken about half a round trip earlier, so pacing on it overstates the
    // queue and brakes against audio already played. The periodic status
    // projection is ordered and already counts what has been sent since, so
    // in-flight audio must not be added again on top of it.
    const projectedLatency = getProjectedAudioLatency(libWorker, 0);
...
function sendAudio(libWorker, audioQueueIndex, isFinalChunk) {
  ...
  libWorker.audioInFlightBlocks = libWorker.audioInFlightBlocks || [];
  libWorker.audioInFlightBlocks.push({
    sequence: libWorker.audioBlockSequence,
    frames: audioQueueIndex,
    sentAt: getPerformanceTimestamp()
  });
