...
import { computeProducerDelayMs, DEFAULT_TARGET_LATENCY_SECONDS } from '../../audio/pacing';
...
function executeAndCheckAudio(libWorker, resolve) {
  ...
  if (response === 1) {
    ...
    const delayInMilli = computeProducerDelayMs(libWorker.currentAudioLatencyInSeconds, getTargetAudioLatency(libWorker));

    // Recorded so getAudioDiagnostics can show whether the queue reading ever
    // reaches the producer. Backpressure that is wired but never arriving and
    // backpressure that is arriving but too weak look identical from outside.
