...
// Hardware powers on with indeterminate RAM, but emulating that breaks games
// that assume it starts clear, and makes every run differ from the last, which
// golden-frame tests and save states depend on. Off unless asked for.
const randomizeMemoryRange = (wasmByteMemory, start, size) => {
  const end = start + size;
  for (let i = start; i < end; i++) {
    wasmByteMemory[i] = Math.floor(Math.random() * 0x100);
  }
};
...
export function memoryWorkerOnMessage(libWorker, event) {
  ...
  switch (eventData.message.type) {
    case WORKER_MESSAGE_TYPE.CLEAR_MEMORY: {
      ...
      if (eventData.message.randomizeStartupRam) {
        randomizeMemoryRange(libWorker.wasmByteMemory, libWorker.WASMBOY_VIDEO_RAM_LOCATION, libWorker.WASMBOY_VIDEO_RAM_SIZE);
        randomizeMemoryRange(libWorker.wasmByteMemory, libWorker.WASMBOY_WORK_RAM_LOCATION, libWorker.WASMBOY_WORK_RAM_SIZE);
      }
    ...
    case WORKER_MESSAGE_TYPE.GET_CONSTANTS: {
      ...
      (libWorker.WASMBOY_GAME_RAM_BANKS_LOCATION = libWorker.wasmInstance.exports.CARTRIDGE_RAM_LOCATION.valueOf()),
        ...
        (libWorker.WASMBOY_VIDEO_RAM_SIZE = libWorker.wasmInstance.exports.VIDEO_RAM_SIZE.valueOf()),
        (libWorker.WASMBOY_VIDEO_RAM_LOCATION = libWorker.wasmInstance.exports.VIDEO_RAM_LOCATION.valueOf()),
        (libWorker.WASMBOY_WORK_RAM_SIZE = libWorker.wasmInstance.exports.WORK_RAM_SIZE.valueOf()),
        (libWorker.WASMBOY_WORK_RAM_LOCATION = libWorker.wasmInstance.exports.WORK_RAM_LOCATION.valueOf()),
