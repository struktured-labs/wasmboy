...
import { runWasmExportFromWorker, getWasmMemorySectionFromWorker, getWasmConstantFromWorker } from './worker';
...
export const runWasmExport = async (exportKey, parameters, timeout) => {
  return runWasmExportFromWorker(WasmBoyLib.worker, exportKey, parameters, timeout);
};

export const getWasmMemorySection = async (start, end) => {
  return getWasmMemorySectionFromWorker(WasmBoyLib.worker, start, end);
};

export const getWasmConstant = async constantKey => {
  return getWasmConstantFromWorker(WasmBoyLib.worker, constantKey);
};
